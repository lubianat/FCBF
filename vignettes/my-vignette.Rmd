...
This normalized single cell expression can be used in machinel learning (ML) models to obtain classifiers for
...
There is no consensus of the optimal way of discretizing gene expression, and this is even more true to single cell RNA-Seq. The discretize_exprs function simply gets the range of expression (min to max) for each gene and assigns the first third the label of "low" and, to the other two, the label of "high". 
In this way, all the zeroes and low expression values are assigned the label "low" in a gene-dependent manner.
Notably, this step is only done for variable selection, and the models can be run with the original dataset.
...
"Error in fcbf(discrete_expression, target, verbose = TRUE) : No prospective features for this threshold level. Threshold: 0.25"

That is because the built-in threshold for the SU, 0.25, is too high for this data set. You can use the function 'su_plot' to decide for yourself which treshold is the best
...
```
In the end, this process has selected a very low number of variables in comparison with the orginal dataset. 
...
We note that other techniques based on predominant correlation might be better for heterogeneous dataset. FCBF provides a interpretable and robust option, with results that are genreally good.
